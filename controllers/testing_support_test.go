...
const (
	...
	testCertificatePath   = "../config/samples/vllm-ca-certs/ca-bundle.crt"
)
...
// loadTestCertificate loads a valid test certificate from the samples directory.
func loadTestCertificate(t *testing.T) string {
	t.Helper()
	// Read the test certificate generated by generate_certificates.sh
	data, err := os.ReadFile(testCertificatePath)
	if err != nil {
		// If the file doesn't exist, generate it
		t.Logf("Test certificate not found, attempting to generate it")
		cmd := exec.Command("bash", "../config/samples/generate_certificates.sh")
		cmd.Dir = "../config/samples"
		if runErr := cmd.Run(); runErr != nil {
			t.Fatalf("Failed to generate test certificate: %v", runErr)
		}
		// Try reading again
		data, err = os.ReadFile(testCertificatePath)
		require.NoError(t, err, "Failed to read test certificate after generation")
	}
	return string(data)
}
